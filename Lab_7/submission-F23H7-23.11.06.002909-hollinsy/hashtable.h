...
private:
    ...
    int search(const K& key) const {
        ...
        Hasher hasher;
        int index = hasher(key) % this->buckets.size();
//        cout << "indexing: " << key << endl;
        for (int i = 0; i < (int)this->buckets.size(); ++i) {
//            cout << "  index: " << index;
//            cout << "\tvalue: " << this->buckets[index % this->buckets.size()].val << "\tkey: "
//                << this->buckets[index % this->buckets.size()].key << "\t";
//            if (this->buckets[index].status == Status::Empty) cout << "\tEmpty" << endl;
//            if (this->buckets[index].status == Status::Occupied) cout << "Occupied" << endl;
//            if (this->buckets[index].status == Status::Deleted) cout << "Deleted" << endl;
...
//                cout << "Return -1" << endl;
    ...
    int newKey(const K& key) {
        Hasher h;
        if (this->buckets.size() < this->num_elements*2) this->rehash_and_grow();
        int index = h(key) % this->buckets.size();;
